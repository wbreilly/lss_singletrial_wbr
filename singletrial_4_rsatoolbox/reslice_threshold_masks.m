...
subjects    = {'s002' 's003' 's004' 's007' 's008' 's009' 's010' 's011'};
b.scriptdir = scriptdir;

newnames = {'thresh_hipp_body_L.nii' 'thresh_hipp_body_R.nii'  'thresh_hipp_head_L.nii' 'thresh_hipp_head_R.nii' ...
            'thresh_hipp_tail_L.nii' 'thresh_hipp_tail_R.nii' 'thresh_phc_ant_L.nii' 'thresh_phc_ant_R.nii' ...
            'thresh_prc_L.nii' 'thresh_prc_R.nii' 'thresh_VMPFC.nii'};

% Loop over subjects
for isub = 1:length(subjects)
    b.curSubj   = subjects{isub};
    b.dataDir   = fullfile(dataDir, b.curSubj, 'ANTS_MTL_1');
    ...
    for imask = 1:size(b.masks,1)
        ...
        matlabbatch{imask}.spm.spatial.coreg.write.roptions.interp = 4;
    ...
    for imask = 1:size(b.sliced,1) 
        ...
        spm_imcalc(char(input_img),char(output_img), 'i1 > .2'); 
    end % imask
end % end isubd